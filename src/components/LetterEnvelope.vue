...
import { useLetterModalStore } from '@/stores/useLetterModalStore'
import { ref } from 'vue'
import Letter from './LetterModal.vue'

const isEnvelopeOpen = ref<boolean>(false)
const isLetterLifted = ref<boolean>(false)

const letterModalStore = useLetterModalStore()
...
const handleClick = () => {
  ...
  if (isEnvelopeOpen.value && isLetterLifted.value) {
    letterModalStore.openModal()
  }
}
</script>
  ...
  >
    ...
    >
      <p class="text-justify p-4 max-md:text-sm">
        ...
        <br />
        <br />
        Esse é o nosso primeiro dia dos namorados. É o meu primeiro da vida, honestamente, mas eu
        não poderia estar mais contente. Sou imensamente grato por ter te conhecido, ter você ao meu
        lado é, de longe, a minha maior realização dos últimos anos.
      </p>
    </div>
  </div>
</template>
